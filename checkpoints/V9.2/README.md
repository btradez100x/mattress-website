...
Superseded filename. Same package as **9.2.0-order-status-app-proxy**.

Use `checkpoints/9.2.0-order-status-app-proxy/Valtora-Shopify-Theme-9.2.0-order-status-app-proxy.zip` to upload.
...
- `Valtora-Shopify-Theme-V9.2.zip` — upload to Shopify (Online Store → Themes → Add theme → Upload zip)
...
Restore:
```bash
tar -xzf checkpoints/V9.2/preview-and-theme.tar.gz
```
