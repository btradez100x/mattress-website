...
- Parallel zip-handle rename (`size`/`need`/…) was reverted; plan requires professional handles. Live `shopify-theme` `0a72a59` matches.
...
- Checkpoint: `checkpoints/13.0.0-how-it-works-copy/`. Deploy: `v9` `b9379e6`, `shopify-theme` `ea6dc46`. No VERSION bump.
...
- Checkpoint: `checkpoints/13.0.0-heading-weight/`. Deploy: `v9` `7d65775`, `shopify-theme` `84e58b1`. No VERSION bump.

# Regression results — Tuesday 8 Sep 2026 (heading weight)

- Heading weight lives in Theme settings → Brand. Smoke exit 1. Same pre-existing fails: footer lockup, trade [D-reply], password 404, section grounds, lifestyle/press. Checkout cream passed.
- Checkpoint: `checkpoints/13.0.0-heading-weight/`. Deploy: `v9` `1162a1b`, `shopify-theme` `e7592bd`. No VERSION bump.
...
# Regression results — Tuesday 8 Sep 2026 (font set weight)

- Checkpoint: `checkpoints/13.0.0-font-set-weight/`. Deploy: `v9` `300def6`, `shopify-theme` `b75c744`. Font set switches heading weight. Bold is the guidelines default. Classic is the lighter page. No VERSION bump.
...
- Scope note: Lockup, Trading as, Styled by, founder/FAQ/manufacturing/landing copy, and share title read Theme settings → Brand name. Ember ampersand is a checkbox again. Duplicate lockup name fields removed. Collage image fallbacks, press, and password WIP left out. Checkpoint: `checkpoints/13.0.0-brand-name-fields/`. Deploy: `v9` `797870b`, `shopify-theme` `2e3721d`. No VERSION bump.
...
- Scope note: Live header now sticks like the preview. Shopify’s header group was only as tall as the bar, so `position: sticky` on `.site-header` had no room. The group is flattened outside the theme editor and the header section sticks. Announcement still scrolls away. Checkpoint: `checkpoints/13.0.0-sticky-header/`. Deploy: `v9` `5bb7b9d`, `shopify-theme` `cb5970c`. Hard-refresh and scroll. No VERSION bump.
...
## Smoke
- Command: `./scripts/regression-smoke.sh`
- Result: Logo lockup checks **PASSED**. Overall smoke **FAILED** (6 checks), all pre-existing uncommitted WIP: missing password form path in working tree (restored for this deploy); Trade `[D-reply]` / 5 working days; market-tagline helper; float Checkout cream-on-navy; lifestyle webp / press priority. Consistency: 941 passed, 2 failed (Trade).
- Scope note: Deployed Onni & Osher live-text lockup (header two-line, footer three-line reverse, password centred). Theme settings → Logo lockup (site vs V3 palettes, symbol colour none/stone/ember). Checkpoint: `checkpoints/13.0.0-onni-osher-logo-lockup/`. Deploy: `v9` `231ae5e`, `shopify-theme` `30b7177`. Hard-refresh header/footer. No VERSION bump.
...
## Smoke
- Command: `./scripts/regression-smoke.sh`
- Result: **PASSED** (exit 0), including navy/gold live tokens and cream-on-dark lock.
